...
| Common response envelopes and slim paths | Done | List envelopes are common (`src/filigree/mcp_tools/common.py`), batch envelopes are common, stats include canonical and compatibility keys (`src/filigree/db_meta.py`). MCP and CLI JSON `get_valid_transitions` now return `ListResponse[TransitionDetail]` instead of bare arrays (`src/filigree/mcp_tools/workflow.py`; `src/filigree/cli_commands/workflow.py`). MCP `get_plan` and CLI JSON `plan/get-plan` default to slim issue records and accept `response_detail="full"` / `--detail full` for full issue payloads (`src/filigree/mcp_tools/planning.py`; `src/filigree/cli_commands/planning.py`). | None. |
...
| Plan editing and plan-read ergonomics | Done | High-level plan operations exist: `create_plan_from_file`, `add_plan_step`, `retarget_plan_dependency`, `move_plan_step`, `label_plan_tree`, and `label_subtree` (`src/filigree/mcp_tools/planning.py`). Plan dependency reference validation rejects ambiguous JSON values. `get_plan` now has slim/full response detail, and `move_plan_step` warns when active dependency edges carry forward so callers can retarget blockers explicitly. There is no plan-specific compaction command; event compaction remains an archive-maintenance operation. | None. |
...
| File timelines include issue events | Done | `get_file_timeline` supports `include_issue_events` and `event_type='issue_event'` (`src/filigree/db_files.py:1677`; `uv run filigree get-file-timeline --help`). Tests cover CLI and core behavior (`tests/core/test_files.py:2352`; `tests/cli/test_files_commands.py:382`). | None. |
