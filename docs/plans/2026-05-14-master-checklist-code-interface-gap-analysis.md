...
| Observation triage and session cleanup filters | Partial | MCP/CLI list filters exist for actor, file, priority, age, sort, and direction (`src/filigree/mcp_tools/observations.py:66`; `uv run filigree list-observations --help`). Batch dismiss and batch promote exist (`src/filigree/mcp_tools/observations.py:137`, `src/filigree/mcp_tools/observations.py:204`). | No first-class link/duplicate/merge disposition for observations into existing issues, and no promote-many-into-one or filtered batch action. |
...
- Observation duplicate/link/merge dispositions.
